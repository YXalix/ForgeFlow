...
pub enum Commands {
    ...
    ///
    /// Examples:
    ///   vkt config                    # Show all configuration
    ///   vkt config user.name          # Get specific value
    ///   vkt config user.name "John"   # Set specific value
    ///   vkt config --setup            # Run interactive setup
    Config(ConfigArgs),
}
...
pub struct ConfigArgs {
    /// Config key in dotted notation (e.g., user.name)
    #[arg(value_name = "KEY", help = "Configuration key (e.g., user.name, remote.token)")]
    pub key: Option<String>,

    /// Config value to set
    #[arg(value_name = "VALUE", help = "Value to set (omit to get current value)")]
    pub value: Option<String>,

    /// List all configuration values
    #[arg(short, long, help = "List all configuration values")]
    pub list: bool,
...
mod tests {
    ...
    #[test]
    fn test_config_command_no_args() {
        let cli = parse_args_from(&["vkt", "config"]).unwrap();
        match cli.command {
    ...
    fn test_config_command_get() {
        let cli = parse_args_from(&["vkt", "config", "user.name"]).unwrap();
        match cli.command {
            Commands::Config(args) => {
                assert_eq!(args.key, Some("user.name".to_string()));
                assert!(args.value.is_none());
            }
            _ => panic!("Expected Config command"),
        }
    ...
    fn test_config_command_set() {
        let cli = parse_args_from(&["vkt", "config", "user.name", "John Doe"]).unwrap();
        match cli.command {
            Commands::Config(args) => {
                assert_eq!(args.key, Some("user.name".to_string()));
                assert_eq!(args.value, Some("John Doe".to_string()));
            }
            _ => panic!("Expected Config command"),
        }
    }
    ...
    #[test]
    fn test_config_command_list_flag() {
        let cli = parse_args_from(&["vkt", "config", "--list"]).unwrap();
        match cli.command {
            Commands::Config(args) => {
                assert!(args.list);
            }
    ...
    #[test]
    fn test_config_command_setup_flag() {
        let cli = parse_args_from(&["vkt", "config", "--setup"]).unwrap();
        match cli.command {
            Commands::Config(args) => {
                assert!(args.setup);
            }
